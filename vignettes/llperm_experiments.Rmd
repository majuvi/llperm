...
```{r}
set.seed(42)
n.simulations <- 1 # simulate data and repeat the experiments n times to reduce sampling variance
  ...
  for (effect in effects) {
    ...
    for (model in names(models)) {
      ...
      results.model <- foreach (i=1:length(otu.names), .combine=rbind) %dopar% {
        ...
        fit <- prr.test.ll(eq, otu.simulated, test.var="Group", test="count", n.rep=500, family=family)
